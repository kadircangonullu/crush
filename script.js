...
function renderFullMember(index, animate = true) {
  ...
  memberTrack.classList.toggle("is-dragging", !animate);
  memberTrack.style.transform = `translate3d(${-fullMemberIndex * 100}%,0,0)`;
  ...
  memberSlides.forEach((slide, i) =>
    slide.setAttribute("aria-hidden", i === fullMemberIndex ? "false" : "true"),
  );
}
...
memberViewport?.addEventListener("pointermove", (event) => {
  ...
  const viewportWidth = memberViewport.clientWidth || 1;
  const resisted = dx * (Math.abs(dx) > viewportWidth * 0.65 ? 0.55 : 1);
  memberTrack.style.transform = `translate3d(calc(${-fullMemberIndex * 100}% + ${resisted}px),0,0)`;
});
