(async function bootCrushSite() {
  ...
  (function startCrushAnalytics() {
    if (
      !cmsReady ||
      !cmsDb ||
      !window.CRUSH_SUPABASE?.url ||
      !window.CRUSH_SUPABASE?.publishableKey
    )
      return;
    ...
    try {
      alreadyStarted = sessionStorage.getItem(startedKey) === "1";
    } catch (_) {}
    ...
    if (!alreadyStarted) {
      cmsDb.functions
        .invoke("track-visit", { body: { event: "start", ...baseBody } })
        .then(({ error }) => {
          if (!error) {
            try {
  ...
  function renderCmsMembers(members) {
    ...
    if (track) {
      track.innerHTML = members
        .map((m, i) => {
          ...
          const tags = (m.tags || [])
            .map((tag) => `<span>${cmsEsc(tag)}</span>`)
            .join("");
          const socials = [
            memberSocialLink(
              m.instagram_url,
              `${m.display_name} Instagram`,
              "◎",
            ),
    ...
    if (kicker)
      kicker.textContent = `CRUSH MEMBERS · 01—${String(members.length).padStart(2, "0")}`;
  }
  ...
  function videoThumb(v, quality = "hqdefault") {
    return cmsUrl(
      v.thumbnail_url,
      v.youtube_id
        ? `https://img.youtube.com/vi/${encodeURIComponent(v.youtube_id)}/${quality}.jpg`
        : "",
    );
  }
  ...
  function renderCmsVideos(videos) {
    ...
    const homepage = videos.filter(
      (v) => v.section === "homepage" || v.section === "both",
    );
    const archive = videos.filter(
      (v) => v.section === "archive" || v.section === "both",
    );

    const grid = document.querySelector("#video .video-grid");
    if (grid && homepage.length) {
      grid.innerHTML = homepage
        .map(
          (v) => `
  ...
  function cmsDateParts(value) {
    ...
    return {
      ...
      monthShort: new Intl.DateTimeFormat("tr-TR", { month: "short" })
        .format(date)
        .replace(".", "")
        .toLocaleUpperCase("tr-TR"),
      monthLong: new Intl.DateTimeFormat("tr-TR", { month: "long" })
        .format(date)
        .toLocaleUpperCase("tr-TR"),
      weekday: new Intl.DateTimeFormat("tr-TR", { weekday: "long" })
        .format(date)
        .toLocaleUpperCase("tr-TR"),
    };
  }
  ...
  function renderCmsEvents(events) {
    ...
    track.querySelectorAll("[data-event-url]").forEach((card) => {
      const open = () =>
        window.open(card.dataset.eventUrl, "_blank", "noopener,noreferrer");
    ...
    if (kicker && events?.length) {
      const first = cmsDateParts(events[0].event_date),
        last = cmsDateParts(events[events.length - 1].event_date);
      kicker.textContent =
        first && last && first.monthLong === last.monthLong
          ? `${first.monthLong} ${first.date.getFullYear()} · İÇERİK TAKVİMİ`
          : "CRUSH · İÇERİK TAKVİMİ";
    }
    updateCalendar();
  }
  ...
  function renderCmsConcerts(concerts) {
    ...
    if (!list) return;
    list.innerHTML = (concerts || [])
      .map((concert, i) => {
        const p = cmsDateParts(concert.concert_date);
        if (!p) return "";
        const time = String(concert.concert_time || "").slice(0, 5);
        const status =
          concert.status_text ||
          concertStatusText[concert.status] ||
          String(concert.status || "").toLocaleUpperCase("tr-TR");
  ...
  function renderCmsNews(news) {
    ...
    grid.innerHTML = (news || [])
      .map((item) => {
        const p = cmsDateParts(item.published_at);
        if (!p) return "";
  ...
  function renderCmsAnnouncement(items) {
    ...
    const link = item.url
      ? String(item.url).startsWith("#")
        ? `<a href="${cmsEsc(item.url)}">Detayları gör →</a>`
        : `<a href="${cmsEsc(item.url)}" rel="noopener noreferrer" target="_blank">Detayları gör →</a>`
      : "";
  ...
  function renderFeaturedTrack(track) {
    ...
    document
      .querySelectorAll("#nowPlaying img, #recordSleeve img")
      .forEach((img) => {
        img.src = cover;
    ...
    if (label)
      label.textContent = `${track.title || "CRUSH!"} — ${track.subtitle || "CRUSH"}`;
    ...
    if (audioEl && audio) {
      ...
      source.src = audio;
      source.type = audio.toLowerCase().includes(".m4a")
        ? "audio/mp4"
        : "audio/mpeg";
  ...
  function renderCmsSiteProfile(profile) {
    ...
    if (primary && profile.hero_primary)
      primary.textContent = profile.hero_primary;
    if (secondary && profile.hero_secondary)
      secondary.textContent = profile.hero_secondary;
    const previewHeading = document.querySelector(
      "#uyeler-preview .panel-head h2",
    );
    if (previewHeading && (profile.hero_primary || profile.hero_secondary))
      previewHeading.textContent =
        [profile.hero_primary, profile.hero_secondary]
          .filter(Boolean)
          .join(". ") + ".";

    const storyKicker = document.querySelector(".story-copy > .section-kicker");
    if (storyKicker && profile.story_kicker)
      storyKicker.textContent = profile.story_kicker;
    ...
    if (storyTitle && (profile.story_title_line1 || profile.story_title_em)) {
      ...
      storyTitle.append(
        document.createTextNode(profile.story_title_line1 || ""),
      );
      storyTitle.append(document.createElement("br"));
      const em = document.createElement("em");
      em.textContent = profile.story_title_em || "";
      storyTitle.append(em);
    }
    const storyParagraphs = document.querySelectorAll(".story-text > p");
    [
      profile.story_paragraph_1,
      profile.story_paragraph_2,
      profile.story_paragraph_3,
    ].forEach((text, i) => {
      if (text && storyParagraphs[i]) storyParagraphs[i].textContent = text;
    });
    const groupPhoto = document.querySelector(".story-photo-card > img");
    if (groupPhoto && profile.group_photo_url)
      groupPhoto.src = profile.group_photo_url;
  ...
  function renderCmsSocialLinks(links) {
    ...
    const byPlatform = Object.fromEntries(
      links.map((x) => [String(x.platform || "").toLowerCase(), x]),
    );
    const headerTargets = {
      instagram: "Instagram",
      youtube: "YouTube",
    ...
    Object.entries(headerTargets).forEach(([platform, aria]) => {
      ...
      const el = document.querySelector(
        `.header-actions a[aria-label="${aria}"]`,
      );
      if (!el) return;
      if (link) {
        el.href = link.url;
  ...
  async function hydrateCrushCms() {
    ...
    try {
      ...
      console.warn(
        "CRUSH CMS içeriği yüklenemedi; statik fallback kullanılıyor:",
        error,
      );
  ...
  function stopRecord() {
    if ((!playing && !animating) || hero.classList.contains("returning"))
      return;
    playing = false;
    animating = true;
    setAudio(false);
  ...
  function finishMemberSwipe(event) {
    if (!memberDragging || (event && event.pointerId !== memberPointerId))
      return;
  ...
  function letterCanvasHasDrawing() {
    ...
    try {
      const pixels = canvasCtx.getImageData(
        0,
        0,
        letterCanvas.width,
        letterCanvas.height,
  ...
  senderForm?.addEventListener("submit", async (event) => {
    ...
    if (!message) {
      if (letterSubmitNote)
        letterSubmitNote.textContent =
          "Mektubun boş görünüyor. Önce birkaç kelime yazmalısın.";
      letterEditor?.focus();
      return;
    }
    if (message.length > 6000) {
      if (letterSubmitNote)
        letterSubmitNote.textContent =
          "Mektup en fazla 6000 karakter olabilir.";
      return;
    }
    ...
    if (!window.CRUSH_DB_READY || !window.crushSupabase) {
      if (letterSubmitNote)
        letterSubmitNote.textContent =
          "Post Office henüz sunucuya bağlanmadı. Yönetici Supabase ayarlarını tamamlamalı.";
      return;
    }
    ...
    if (letterSubmitNote)
      letterSubmitNote.textContent =
        "Mektubun CRUSH Post Office'e gönderiliyor…";

    try {
      const drawingDataUrl = letterCanvasHasDrawing()
        ? letterCanvas.toDataURL("image/png")
        : "";
      const { data, error } = await window.crushSupabase.functions.invoke(
        "submit-letter",
        {
          body: {
      ...
      if (letterSubmitNote)
        letterSubmitNote.textContent = "Mektubun CRUSH'a ulaştı ♡ Teşekkürler!";
      ...
      if (letterSubmitNote)
        letterSubmitNote.textContent =
          error?.message ||
          "Mektup gönderilemedi. Lütfen biraz sonra tekrar dene.";
    } finally {
...
const letterPrivacyToggle = document.getElementById("letterPrivacyToggle");
const letterPrivacyText = document.getElementById("letterPrivacyText");

letterPrivacyToggle?.addEventListener("click", () => {
  if (!letterPrivacyText) return;

  const willOpen = letterPrivacyText.hidden;

  letterPrivacyText.hidden = !willOpen;
  letterPrivacyToggle.setAttribute("aria-expanded", String(willOpen));

  letterPrivacyToggle.classList.toggle("is-open", willOpen);
});
